...
class DeepSeekProvider:
    ...
    async def chat(
        ...
        **opts: object,
        ...
        body = _build_deepseek_body(messages, tools, model, opts)
        ...
        return _parse_deepseek_response(response.json())
        ...
        **opts: object,
        ...
        async with self._client.stream(
            ...
            async for line in response.aiter_lines():
                ...
                chunk = _parse_deepseek_sse(data, idx)
                if chunk:
                    yield chunk
                    idx += 1
...
def _parse_deepseek_response(data: Any) -> LLMResponse:
...
def _parse_deepseek_sse(data: Any, idx: int) -> StreamChunk | None:
    if not isinstance(data, dict):
        return None
    choices_raw = data.get("choices", [])
